...
| Resource exhaustion | per-token 6000 req/min; per-user 6000 calls/min, 300 renders/min, 120 exports/min, 12 concurrent calls (an abuse ceiling, not a budget — an agent building an animation makes hundreds of small edits); batches ≤ 200; frames ≤ 16 per sheet; images ≤ 2400 px; project size capped by `MAX_PROJECT_BYTES`; JSON bodies ≤ 4 MB |

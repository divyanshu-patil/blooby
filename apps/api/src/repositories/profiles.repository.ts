 ...
 * write below evicts the row — on EVERY instance, through Redis when there is one — so a
 * role change or a What's New tick is visible at once wherever the next request lands.
...
const evict = shared('profile', cached.forget);
...
export const profilesRepository = {
  ...
  setRole: (id: string, role: UserRole) =>
    prisma.profile.update({ where: { id }, data: { role } }).finally(() => evict(id)),

  setLastSeenRelease: (id: string, version: string) =>
    prisma.profile.update({ where: { id }, data: { lastSeenRelease: version } }).finally(() => evict(id)),

  touchLogin: (id: string) =>
    prisma.profile.update({ where: { id }, data: { lastLoginAt: new Date() } }).finally(() => evict(id)),

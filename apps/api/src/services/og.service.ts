...
import { BRAND_MARK_PNG } from './brandMark.js';
 ...
 *
 * The Icon Composer export rather than the flat shapes: this card is the picture a link
 * unfurls with, and it should carry the icon people actually see on their home screen.
 * `BRAND_MARK_PNG` is generated by `pnpm icons` so nothing here reaches across the repo
 * for a file — a relative path out of apps/api breaks the moment the server is built
 * rather than run from source.
 */
const markAt = (x: number, y: number, size: number) =>
  `<image href="${BRAND_MARK_PNG}" x="${x}" y="${y}" width="${size}" height="${size}"/>`;

...
// --- v0 → current --------------------------------------------------------------
{
  ...
  it('and every step runs on it', check(applied.length === 13, applied.join(', ')));
...
})) as unknown as Project;
{
  ...
  it('a v2 document runs only the steps after it', check(from === 2 && applied.join() === 'freeform layers,showcase presets,several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets,built-in presets are stored by reference', applied.join()));
...
// --- v3 → v4: showcase presets ----------------------------------------------------
/** A v3 document with the library it was saved with — before the showcase presets. */
const v3Library = () => JSON.parse(JSON.stringify({
  name: 'Library', schemaVersion: 3, fps: 30, expressions: [], composition: { width: 720, height: 720 },
  presets: [
    { id: 'p_idle', name: 'Idle', source: 'builtin', durationMs: 900, tracks: [], modifiers: [] },
    { id: 'p_hii', name: 'My own Hii', source: 'custom', durationMs: 500, tracks: [], modifiers: [] },
  ],
})) as unknown as Project;
{
  const { project, applied } = migrateProject(v3Library());
  const ids = project.presets.map((x) => x.id);
  it('a v3 document runs only the steps after it', check(applied.join() === 'showcase presets,several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets,built-in presets are stored by reference', applied.join()));
  ...
  it('one it already had is kept as it was, not doubled', check(
    ids.filter((i) => i === 'p_hii').length === 1 && project.presets.find((x) => x.id === 'p_hii')?.name === 'My own Hii'));
  it('and its own presets are still there', check(ids.includes('p_idle')));
...
})) as unknown as Project;
{
  ...
  it('a v4 document runs only the steps after it', check(hidden.applied.join() === 'several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets,built-in presets are stored by reference', hidden.applied.join()));

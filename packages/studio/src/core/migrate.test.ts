...
// --- v0 → current --------------------------------------------------------------
{
  ...
  it('and every step runs on it', check(applied.length === 12, applied.join(', ')));
...
})) as unknown as Project;
{
  ...
  it('a v2 document runs only the steps after it', check(from === 2 && applied.join() === 'freeform layers,showcase presets,several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets', applied.join()));
...
})) as unknown as Project;
{
  ...
  it('a v3 document runs only the steps after it', check(applied.join() === 'showcase presets,several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets', applied.join()));
...
})) as unknown as Project;
{
  ...
  it('a v4 document runs only the steps after it', check(hidden.applied.join() === 'several mascots,mascot and text presets,faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets', hidden.applied.join()));

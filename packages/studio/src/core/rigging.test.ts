...
// --- migration: an old mascot gets a face, and looks the same --------------------------
{
  ...
  it('a v6 file runs the faces step', check(applied.join() === 'faces,app screen presets,cinematic presets,app mascot kit,each timeline keeps its own layers,cartoon and character presets,built-in presets are stored by reference', applied.join()));
